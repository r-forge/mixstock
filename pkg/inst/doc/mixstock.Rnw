...
like \code{Program Files\\R\\R-\Rver\\bin} depending
...
\begin{itemize}
  ...
  \item \code{JAGS}: go to \url{http://www-fis.iarc.fr/~martyn/software/jags/}
    ...
    \code{R2jags} (\code{install.packages("R2jags",repos="http://r-forge.r-project.org")}) (\emph{hopefully \code{R2jags} will be rolled into \code{R2WinBUGS} 
      shortly, eliminating a little bit of complexity and the need to 
      download and install a development version of the package})
  \end{itemize}
